...
%    10) Secilmis SCP kodlari icin co-occurrence (Jaccard)
    ...
    % UI renkleri (koyu temada gorunur, acik temada yormaz)
    ui_bg   = [1 1 1];            % input arka plan
    ui_fg   = [0 0 0];            % input yazi rengi
    fig_bg  = [0.94 0.94 0.94];   % pencere arka plan (MATLAB light benzeri)
    btn_bg  = [0.87 0.87 0.9];    % buton arka plan

    % Koyu tema etkisini azaltmak icin root varsayilanlarini set et
    set(0, 'DefaultUicontrolBackgroundColor', ui_bg);
    set(0, 'DefaultUicontrolForegroundColor', ui_fg);
    set(0, 'DefaultUipanelBackgroundColor', fig_bg);
    ...
    edit_height = 0.05; % metni ortalamak icin makul yukseklik
    edit_age_min = uicontrol('Parent', f, ...
                             'Style', 'edit', ...
                             'Units', 'normalized', ...
                             'Position', [0.03 0.535 0.12 edit_height], ...
    ...
    edit_age_max = uicontrol('Parent', f, ...
                             'Style', 'edit', ...
                             'Units', 'normalized', ...
                             'Position', [0.17 0.535 0.12 edit_height], ...
    ...
    edit_patient_id = uicontrol('Parent', f, ...
                                'Style', 'edit', ...
                                'Units', 'normalized', ...
                                'Position', [0.03 0.46 0.26 edit_height], ...
                                'String', '', ...
    ...
    btn_height = 0.045;
    btn_positions = [0.40, 0.33, 0.26, 0.19];

    uicontrol('Parent', f, ...
              'Style', 'pushbutton', ...
              'Units', 'normalized', ...
              'Position', [0.03 btn_positions(1) 0.26 btn_height], ...
              'String', 'Yeniden ciz', ...
    ...
    uicontrol('Parent', f, ...
              'Style', 'pushbutton', ...
              'Units', 'normalized', ...
              'Position', [0.03 btn_positions(2) 0.26 btn_height], ...
    ...
    uicontrol('Parent', f, ...
              'Style', 'pushbutton', ...
              'Units', 'normalized', ...
              'Position', [0.03 btn_positions(3) 0.26 btn_height], ...
    ...
    uicontrol('Parent', f, ...
              'Style', 'pushbutton', ...
              'Units', 'normalized', ...
              'Position', [0.03 btn_positions(4) 0.26 btn_height], ...
    ...
    % Tum UI bilesenlerini tek seferde acik temaya cek
    apply_light_theme(f, ui_bg, ui_fg, btn_bg, fig_bg);
    ...
    function on_plot_selection_changed(src, ~)
        ...
        ax = axes('Parent', f, ...
                  ...
                  'Color', [1 1 1]); % grafik alanini beyaz tut
...
function apply_light_theme(f, ui_bg, ui_fg, btn_bg, fig_bg)
    % apply_light_theme  Qt koyu tema siyah input sorununu azaltmak icin
    % tum uicontrol'leri yeniden renklendirir.
    ctrls = findall(f, 'Type', 'uicontrol');
    for k = 1:numel(ctrls)
        c = ctrls(k);
        style = get(c, 'Style');
        switch style
            case {'pushbutton', 'togglebutton'}
                set(c, 'BackgroundColor', btn_bg, 'ForegroundColor', ui_fg);
            case {'edit', 'listbox', 'popupmenu'}
                set(c, 'BackgroundColor', ui_bg, 'ForegroundColor', ui_fg);
            case {'text', 'checkbox', 'radiobutton'}
                set(c, 'BackgroundColor', fig_bg, 'ForegroundColor', ui_fg);
            otherwise
                % no-op
        end
    end

    % Eksenler ve figure arka planlarini acik renk yap
    axes_list = findall(f, 'Type', 'axes');
    for k = 1:numel(axes_list)
        set(axes_list(k), 'Color', [1 1 1]);
    end
    set(f, 'Color', fig_bg);
end
